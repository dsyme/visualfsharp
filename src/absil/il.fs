...
let _ =
    if logging then dprintn "* warning: Il.logging is on"
...
let lazyMap f (x : Lazy<_>) =
    if x.IsValueCreated then notlazy (f (x.Force()))
    else lazy (f (x.Force()))
...
let splitNameAt (nm : string) idx =
    ...
    (nm.Substring(0, idx)), 
    (if idx < last then nm.Substring(idx + 1, last - idx)
     else "")
...
let memoizeNamespaceRightTable =
    new ConcurrentDictionary<string, string option * string>()
let splitNamespace nm = memoizeNamespaceTable.GetOrAdd(nm, splitNamespaceAux)
...
let splitNamespaceToArray nm =
    memoizeNamespaceArrayTable.GetOrAdd(nm, 
                                        fun nm -> 
                                            let x =
                                                Array.ofList (splitNamespace nm)
                                            x)
...
type LazyOrderedMultiMap<'Key, 'Data when 'Key : equality>(keyf : 'Data -> 'Key, lazyItems : Lazy<'Data list>) =
    ...
    let quickMap =
        lazyItems |> lazyMap (fun entries -> 
                         let t =
                             new Dictionary<_, _>(entries.Length, 
                                                  HashIdentity.Structural)
                         do entries |> List.iter (fun y -> 
                                           let key = keyf y
                                           t.[key] <- y :: (if t.ContainsKey
                                                                   (key) then 
                                                                t.[key]
                                                            else []))
                         t)
    ...
    member self.Add(y) =
        new LazyOrderedMultiMap<'Key, 'Data>(keyf, 
                                             lazyItems 
                                             |> lazyMap (fun x -> y :: x))
    member self.Filter(f) =
        new LazyOrderedMultiMap<'Key, 'Data>(keyf, 
                                             lazyItems 
                                             |> lazyMap (List.filter f))
    member self.Item 
        with get (x) =
...
module SHA1 =
    ...
    let shaRead8 sha =
        ...
        let b =
            if sha.pos >= s.Length then shaAfterEof sha
            else int32 s.[sha.pos]
        sha.pos <- sha.pos + 1
        b
    ...
    let sha1HashBytes s =
        let (_h0, _h1, _h2, h3, h4) =
            sha1Hash { stream = s
                       pos = 0
                       eof = false } // the result of the SHA algorithm is stored in registers 3 and 4
        Array.map byte [| b0 h4
                          b1 h4
                          b2 h4
                          b3 h4
                          b0 h3
...
let AssemblyRefUniqueStampGenerator =
    new UniqueStampGenerator<AssemblyRefData>()
...
let GetReferenceUnifiedVersion data =
    ...
    if not (isMscorlib data) then 
        for ref in AssemblyRefUniqueStampGenerator.Table do
            if System.String.Compare(ref.assemRefName, data.assemRefName) = 0 
               && highest < ref.assemRefVersion then 
...
type ILAssemblyRef(data) =
    ...
    static member Create(name, hash, publicKey, retargetable, version, locale) =
        ILAssemblyRef { assemRefName = name
                        assemRefHash = hash
    ...
    static member FromAssemblyName(aname : System.Reflection.AssemblyName) =
        ...
        let version =
            ...
            | v -> 
                Some
                    (uint16 v.Major, uint16 v.Minor, uint16 v.Build, 
                     uint16 v.Revision)
        
        let retargetable =
            aname.Flags = System.Reflection.AssemblyNameFlags.Retargetable
        ILAssemblyRef.Create
            (aname.Name, None, publicKey, retargetable, version, locale)
    ...
    member aref.QualifiedName =
        ...
        | Some(a, b, c, d) -> 
            ...
            | Some pki -> 
                ...
                let convDigit (digit) =
                    let digitc =
                        if digit < 10 then System.Convert.ToInt32 '0' + digit
...
type ILScopeRef =
    ...
    member x.IsModuleRef =
        match x with
        | ILScopeRef.Module _ -> true
        | _ -> false
    
    member x.IsAssemblyRef =
        match x with
        | ILScopeRef.Assembly _ -> true
        | _ -> false
    
    member x.ModuleRef =
        match x with
        | ILScopeRef.Module x -> x
        | _ -> failwith "not a module reference"
    
    member x.AssemblyRef =
        match x with
        | ILScopeRef.Assembly x -> x
        | _ -> failwith "not an assembly reference"
...
type ILArrayShape =
    ...
    (* lobound/size pairs *)
    member x.Rank =
        (let (ILArrayShape l) = x
         l.Length)
    
    static member SingleDimensional = ILArrayShapeStatics.SingleDimensional
    static member FromRank n =
        if n = 1 then ILArrayShape.SingleDimensional
        else ILArrayShape(List.replicate n (Some 0, None))
...
and ILCallingConvStatics() =
    static let instanceCallConv =
        Callconv(ILThisConvention.Instance, ILArgConvention.Default)
    static let staticCallConv =
        Callconv(ILThisConvention.Static, ILArgConvention.Default)
...
type ILTypeRef =
    ...
    static member Create(scope, enclosing, name) =
        let hashCode =
            hash scope * 17 
            ^^^ (hash enclosing * 101 <<< 1) ^^^ (hash name * 47 <<< 2)
        { trefScope = scope
    ...
    member x.AsBoxedType(tspec : ILTypeSpec) =
        if isNil tspec.tspecInst then 
            ...
            | null -> 
                ...
                r
            | _ -> v
        else ILType.Boxed tspec
    
    override x.GetHashCode() = x.hashCode
    
    override x.Equals(yobj) =
        let y = (yobj :?> ILTypeRef)
        (x.ApproxId = y.ApproxId) && (x.Scope = y.Scope) && (x.Name = y.Name) 
        ...
        member x.CompareTo(yobj) =
            ...
            if c <> 0 then c
                ...
                if c <> 0 then c
                    ...
                    if c <> 0 then c
                    else compare x.Enclosing y.Enclosing
    ...
    member tref.AddQualifiedNameExtensionWithNoShortPrimaryAssembly(basic) =
        ...
        if sco = "" then basic
        else String.concat ", " [ basic; sco ]
    
    member tref.QualifiedNameWithNoShortPrimaryAssembly =
        tref.AddQualifiedNameExtensionWithNoShortPrimaryAssembly
            (tref.BasicQualifiedName)
...
and [<StructuralEquality; StructuralComparison>] ILTypeSpec =
    ...
    static member Create(tref, inst) =
        { tspecTypeRef = tref
          tspecInst = inst }
    
    override x.ToString() =
        x.TypeRef.ToString() + if isNil x.GenericArgs then ""
                               else "<...>"
    
    member x.BasicQualifiedName =
        let tc = x.TypeRef.BasicQualifiedName
        if isNil x.GenericArgs then tc
        else 
            tc + "[" 
            + String.concat "," 
                  (x.GenericArgs 
                   |> List.map 
                          (fun arg -> 
                          "[" + arg.QualifiedNameWithNoShortPrimaryAssembly 
                          + "]")) + "]"
...
and [<RequireQualifiedAccess; StructuralEquality; StructuralComparison>] ILType =
    ...
    member x.BasicQualifiedName =
        ...
        | ILType.Array(ILArrayShape(s), ty) -> 
            ty.BasicQualifiedName + "[" + System.String(',', s.Length - 1) + "]"
        ...
        | ILType.FunctionPointer _mref -> 
            failwith "unexpected function pointer type"
    
    member x.AddQualifiedNameExtensionWithNoShortPrimaryAssembly(basic) =
        match x with
        | ILType.TypeVar _n -> basic
        | ILType.Modified(_, _ty1, ty2) -> 
            ty2.AddQualifiedNameExtensionWithNoShortPrimaryAssembly(basic)
        | ILType.Array(ILArrayShape(_s), ty) -> 
            ty.AddQualifiedNameExtensionWithNoShortPrimaryAssembly(basic)
        | ILType.Value tr | ILType.Boxed tr -> 
            tr.AddQualifiedNameExtensionWithNoShortPrimaryAssembly(basic)
    ...
    member x.QualifiedNameWithNoShortPrimaryAssembly =
        x.AddQualifiedNameExtensionWithNoShortPrimaryAssembly
...
type ILMethodRef =
    ...
    override x.ToString() =
        x.EnclosingTypeRef.ToString() + "::" + x.Name + "(...)"
...
type ILInstr =
    ...
    (* FOR EXTENSIONS, e.g. MS-ILX *) | EI_ilzero of ILType
...
type ILNativeType =
    ...
    (* guid,nativeTypeName,custMarshallerName,cookieString *)
...
type ILOverridesSpec =
    ...
    member x.MethodRef =
        let (OverridesSpec(mr, _ty)) = x
        mr
    
    member x.EnclosingType =
        let (OverridesSpec(_mr, ty)) = x
        ty
...
let typesOfILParams (ps : ILParameters) : ILTypes =
    ps |> List.map (fun p -> p.Type)
...
type ILMethodDef =
    ...
    member x.IsIL =
        match x.mdBody.Contents with
        | MethodBody.IL _ -> true
        | _ -> false
    
    member x.Locals =
        match x.mdBody.Contents with
        | MethodBody.IL il -> il.Locals
        | _ -> []
    ...
    member x.IsClassInitializer =
        match x.mdKind with
        | MethodKind.Cctor -> true
        | _ -> false
    
    member x.IsConstructor =
        match x.mdKind with
        | MethodKind.Ctor -> true
        | _ -> false
    ...
    member x.IsNonVirtualInstance =
        match x.mdKind with
        | MethodKind.NonVirtual -> true
        | _ -> false
    
    member x.IsVirtual =
        match x.mdKind with
        | MethodKind.Virtual _ -> true
        | _ -> false
    
    member x.IsFinal =
        match x.mdKind with
        | MethodKind.Virtual v -> v.IsFinal
        | _ -> invalidOp "not virtual"
    
    member x.IsNewSlot =
        match x.mdKind with
        | MethodKind.Virtual v -> v.IsNewSlot
        | _ -> invalidOp "not virtual"
    
    member x.IsCheckAccessOnOverride =
        match x.mdKind with
        | MethodKind.Virtual v -> v.IsCheckAccessOnOverride
        | _ -> invalidOp "not virtual"
    
    member x.IsAbstract =
        match x.mdKind with
        | MethodKind.Virtual v -> v.IsAbstract
        | _ -> invalidOp "not virtual"
...
type ILMethodDefs(f : unit -> ILMethodDef []) =
    ...
    let mutable dict =
        InlineDelayInit<_>(fun () -> 
            ...
            for i = arr.Length - 1 downto 0 do
                let y = arr.[i]
                let key = y.Name
                if t.ContainsKey key then t.[key] <- y :: t.[key]
                else t.[key] <- [ y ]
            t)
    ...
    member x.FindByName nm =
        if dict.Value.ContainsKey nm then dict.Value.[nm]
        else []
    
    member x.FindByNameAndArity(nm, arity) =
        x.FindByName nm 
        |> List.filter (fun x -> List.length x.Parameters = arity)
...
type ILMethodImplDefs =
    ...
    member x.AsList =
        let (MethodImpls ltab) = x
        Map.foldBack (fun _x y r -> y @ r) (ltab.Force()) []
...
type ILTypeDef =
    ...
    member x.IsClass =
        (match x.tdKind with
         | ILTypeDefKind.Class -> true
         | _ -> false)
    
    member x.IsInterface =
        (match x.tdKind with
         | ILTypeDefKind.Interface -> true
         | _ -> false)
    
    member x.IsEnum =
        (match x.tdKind with
         | ILTypeDefKind.Enum -> true
         | _ -> false)
...
and ILNestedExportedTypes =
    ...
    member x.AsList =
        let (ILNestedExportedTypes ltab) = x
        Map.foldBack (fun _x y r -> y :: r) (ltab.Force()) []
...
and ILExportedTypesAndForwarders =
    ...
    member x.AsList =
        let (ILExportedTypesAndForwarders ltab) = x
        Map.foldBack (fun _x y r -> y :: r) (ltab.Force()) []
...
type ILModuleDef =
    { Manifest : ILAssemblyManifest option
      ...
      NativeResources : list<Lazy<byte []>> }
    ...
    member x.ManifestOfAssembly =
        ...
        | None -> 
            failwith 
                "no manifest.  It is possible you are using an auxiliary module of an assembly in a context where the main module of an assembly is expected.  Typically the main module of an assembly must be specified first within a list of the modules in an assembly."
...
// -------------------------------------------------------------------- 
let mkSimpleAssRef n = ILAssemblyRef.Create(n, None, None, false, None, None)
...
let mkILTypeForGlobalFunctions scoref =
    mkILBoxedType 
        (mkILNonGenericTySpec 
             (ILTypeRef.Create(scoref, [], typeNameForGlobalFunctions)))
...
let mkILMethSpec (mref, vc, tinst, minst) =
    mkILMethSpecForMethRefInTy 
        (mref, mkILNamedTy vc mref.EnclosingTypeRef tinst, minst)
let mk_mspec_in_tref (tref, vc, cc, nm, args, rty, tinst, minst) =
    mkILMethSpec 
        (mkILMethRef (tref, cc, nm, List.length minst, args, rty), vc, tinst, 
         minst)
let mkILMethSpecInTy (typ : ILType, cc, nm, args, rty, minst : ILGenericArgs) =
    mkILMethSpecForMethRefInTy 
        (mkILMethRef (typ.TypeRef, cc, nm, minst.Length, args, rty), typ, minst)
...
let mkILCtorMethSpec (tref, args, cinst) =
    mk_mspec_in_tref 
        (tref, AsObject, ILCallingConv.Instance, ".ctor", args, ILType.Void, 
         cinst, [])
let mkILCtorMethSpecForTy (ty, args) =
    mkILMethSpecInTy 
        (ty, ILCallingConv.Instance, ".ctor", args, ILType.Void, [])
...
let mkILFieldRef (tref, nm, ty) =
    { EnclosingTypeRef = tref
      Name = nm
      Type = ty }

let mkILFieldSpec (tref, ty) =
    { FieldRef = tref
      EnclosingType = ty }
...
let mkILCustomAttrsFromArray (l : ILAttribute []) =
    if l.Length = 0 then emptyILCustomAttrs
    else ILAttributes(fun () -> l)
...
let mkILSecurityDecls l =
    ...
    | _ -> ILPermissions.SecurityDecls l
...
let addILTypeDef td (tdefs : ILTypeDefs) =
    ILTypeDefs(fun () -> 
        [| yield getName (notlazy td)
           yield! tdefs.AsArrayOfLazyTypeDefs |])

let mkILTypeDefsFromArray l =
    ILTypeDefs(fun () -> Array.map (notlazy >> getName) l)
...
// -------------------------------------------------------------------- 
let defaultSubSystem = 3
(* this is what comes out of ILDASM on 30/04/2001 *)
let defaultPhysAlignment = 512
(* this is what comes out of ILDASM on 30/04/2001 *)
let defaultVirtAlignment = 0x2000
(* this is what comes out of ILDASM on 30/04/2001 *)
let defaultImageBase = 0x034f0000
(* this is what comes out of ILDASM on 30/04/2001 *)
...
type ILGlobals(primaryScopeRef) =
    ...
    let m_typ_Object =
        mkILBoxedType (mkILNonGenericTySpec (m_mkSysILTypeRef tname_Object))
    let m_typ_String =
        mkILBoxedType (mkILNonGenericTySpec (m_mkSysILTypeRef tname_String))
    let m_typ_Array =
        mkILBoxedType (mkILNonGenericTySpec (m_mkSysILTypeRef tname_Array))
    let m_typ_Type =
        mkILBoxedType (mkILNonGenericTySpec (m_mkSysILTypeRef tname_Type))
    let m_typ_SByte =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_SByte))
    let m_typ_Int16 =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_Int16))
    let m_typ_Int32 =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_Int32))
    let m_typ_Int64 =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_Int64))
    let m_typ_Byte =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_Byte))
    let m_typ_UInt16 =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_UInt16))
    let m_typ_UInt32 =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_UInt32))
    let m_typ_UInt64 =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_UInt64))
    let m_typ_Single =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_Single))
    let m_typ_Double =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_Double))
    let m_typ_Bool =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_Bool))
    let m_typ_Char =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_Char))
    let m_typ_IntPtr =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_IntPtr))
    let m_typ_UIntPtr =
        ILType.Value(mkILNonGenericTySpec (m_mkSysILTypeRef tname_UIntPtr))
...
let ldargs =
    [| for i in 0..128 -> I_ldarg(uint16 i) |]

let mkLdarg i =
    if 0us < i && i < uint16 ldargs.Length then ldargs.[int i]
    else I_ldarg i

let mkLdarg0 = mkLdarg 0us

let ldlocs =
    [| for i in 0..512 -> I_ldloc(uint16 i) |]

let mkLdloc i =
    if 0us < i && i < uint16 ldlocs.Length then ldlocs.[int i]
    else I_ldloc i

let stlocs =
    [| for i in 0..512 -> I_stloc(uint16 i) |]

let mkStloc i =
    if 0us < i && i < uint16 stlocs.Length then stlocs.[int i]
    else I_stloc i

let ldi32s =
    [| for i in 0..256 -> AI_ldc(DT_I4, ILConst.I4 i) |]

let mkLdcInt32 i =
    if 0 < i && i < ldi32s.Length then ldi32s.[i]
    else AI_ldc(DT_I4, ILConst.I4 i)
...
let ecmaPublicKey =
    PublicKeyToken
        (Bytes.ofInt32Array [| 0xde; 0xad; 0xbe; 0xef; 0xca; 0xfe; 0xfa; 0xce |])
...
let isPrimaryAssemblyTySpec (tspec : ILTypeSpec) n =
    ...
    (tref.Name = n) && match scoref with
                       | ILScopeRef.Assembly n -> 
                           PrimaryAssembly.IsSomePrimaryAssembly n.Name
...
let isILTypedReferenceTy ty =
    isILValuePrimaryAssemblyTy ty "System.TypedReference"
...
let rec rescopeILTypeSpec scoref (tspec1 : ILTypeSpec) =
    ...
    if tref1 === tref2 then 
        if isNil tinst1 then tspec1
        else 
            let tinst2 = rescopeILTypes scoref tinst1
            if tinst1 === tinst2 then tspec1
            else ILTypeSpec.Create(tref2, tinst2)
    else 
        let tinst2 = rescopeILTypes scoref tinst1
...
and rescopeILType scoref typ =
    ...
    | ILType.FunctionPointer t -> 
        ILType.FunctionPointer(rescopeILCallSig scoref t)
    | ILType.Byref t -> ILType.Byref(rescopeILType scoref t)
    | ILType.Boxed cr1 -> 
        let cr2 = rescopeILTypeSpec scoref cr1
        if cr1 === cr2 then typ
        else mkILBoxedType cr2
    | ILType.Array(s, ety1) -> 
        let ety2 = rescopeILType scoref ety1
        if ety1 === ety2 then typ
        else ILType.Array(s, ety2)
    | ILType.Value cr1 -> 
        let cr2 = rescopeILTypeSpec scoref cr1
        if cr1 === cr2 then typ
        else ILType.Value cr2
    | ILType.Modified(b, tref, ty) -> 
        ILType.Modified
            (b, rescopeILTypeRef scoref tref, rescopeILType scoref ty)
    | x -> x
...
and rescopeILCallSig scoref csig =
    mkILCallSig 
        (csig.CallingConv, rescopeILTypes scoref csig.ArgTypes, 
         rescopeILType scoref csig.ReturnType)
...
let rec instILTypeSpecAux numFree inst (tspec : ILTypeSpec) =
    ILTypeSpec.Create
        (tspec.TypeRef, instILGenericArgsAux numFree inst tspec.GenericArgs)

and instILTypeAux numFree (inst : ILGenericArgs) typ =
    match typ with
    | ILType.Ptr t -> ILType.Ptr(instILTypeAux numFree inst t)
    | ILType.FunctionPointer t -> 
        ILType.FunctionPointer(instILCallSigAux numFree inst t)
    ...
    | ILType.TypeVar v -> 
        let v = int v
        let top = inst.Length
        if v < numFree then typ
        else if v - numFree >= top then ILType.TypeVar(uint16 (v - top))
        else List.item (v - numFree) inst
    | x -> x

and instILGenericArgsAux numFree inst i =
    List.map (instILTypeAux numFree inst) i

and instILCallSigAux numFree inst csig =
    mkILCallSig 
        (csig.CallingConv, List.map (instILTypeAux numFree inst) csig.ArgTypes, 
         instILTypeAux numFree inst csig.ReturnType)
...
let mkMethodBody (zeroinit, locals, maxstack, code, tag) =
    MethodBody.IL(mkILMethodBody (zeroinit, locals, maxstack, code, tag))
// -------------------------------------------------------------------- 
// Make a constructor
// -------------------------------------------------------------------- 
...
let mkILNonGenericEmptyCtor tag superTy =
    ...
    mkILCtor 
        (ILMemberAccess.Public, [], 
         mkMethodBody (false, [], 8, nonBranchingInstrsToCode ctor, tag))
...
let mk_ospec (typ : ILType, callconv, nm, genparams, formal_args, formal_ret) =
    OverridesSpec
        (mkILMethRef 
             (typ.TypeRef, callconv, nm, genparams, formal_args, formal_ret), 
         typ)

let mkILGenericVirtualMethod (nm, access, genparams, actual_args, actual_ret, 
                              impl) =
    { Name = nm
      GenericParams = genparams
      CallingConv = ILCallingConv.Instance
      mdKind =
          MethodKind.Virtual { IsFinal = false
...
let mkILNonGenericVirtualMethod (nm, access, args, ret, impl) =
    mkILGenericVirtualMethod 
        (nm, access, mkILEmptyGenericParams, args, ret, impl)

let mkILGenericNonVirtualMethod (nm, access, genparams, actual_args, actual_ret, 
                                 impl) =
    { Name = nm
      GenericParams = genparams
      CallingConv = ILCallingConv.Instance
...
let cdef_cctorCode2CodeOrCreate tag f cd =
    ...
    let cctor =
        ...
        | [] -> 
            mkILClassCtor 
                (mkMethodBody (false, [], 1, nonBranchingInstrsToCode [], tag))
...
let mkRefToILMethod (tref, md : ILMethodDef) =
    mkILMethRef 
        (tref, md.CallingConv, md.Name, md.GenericParams.Length, 
         md.ParameterTypes, md.Return.Type)
let mkRefToILField (tref, fdef : ILFieldDef) =
    mkILFieldRef (tref, fdef.Name, fdef.Type)
...
let mkRefForILField scope (tdefs, tdef) (fdef : ILFieldDef) =
    mkILFieldRef 
        (mkRefForNestedILTypeDef scope (tdefs, tdef), fdef.Name, fdef.Type)
(* Creates cctor if needed *)
                     ...
                     @ List.concat 
                           (List.mapi 
                                ...
                                [ mkLdarg0
                                  ...
                                  mkNormalStfld 
                                      (mkILFieldSpecInTy (typ, nm, ty)) ]) flds)), 
              tag))

let mkILSimpleStorageCtorWithParamNames (tag, base_tspec, typ, extraParams, flds, 
                                         access) =
    let preblock =
    ...
    mkILStorageCtorWithParamNames 
        (tag, preblock, typ, extraParams, flds, access)

let addParamNames flds = flds |> List.map (fun (nm, ty) -> (nm, nm, ty))
let mkILSimpleStorageCtor (tag, base_tspec, typ, extraParams, flds, access) =
    mkILSimpleStorageCtorWithParamNames 
        (tag, base_tspec, typ, extraParams, addParamNames flds, access)
let mkILStorageCtor (tag, preblock, typ, flds, access) =
    mkILStorageCtorWithParamNames 
        (tag, preblock, typ, [], addParamNames flds, access)

let mkILGenericClass (nm, access, genparams, extends, impl, methods, fields, 
                      nestedTypes, props, events, attrs, init) =
...
let mkRawDataValueTypeDef (iltyp_ValueType : ILType) (nm, size, pack) =
    { tdKind = ILTypeDefKind.ValueType
      ...
      IsSpecialName = false
      Layout =
          ILTypeDefLayout.Explicit { Size = Some size
                                     Pack = Some pack }
      Encoding = ILDefaultPInvokeEncoding.Ansi
...
let mkILSimpleClass (ilg : ILGlobals) 
    (nm, access, methods, fields, nestedTypes, props, events, attrs, init) =
    mkILGenericClass 
        (nm, access, mkILEmptyGenericParams, ilg.typ_Object, [], methods, fields, 
         nestedTypes, props, events, attrs, init)
let mkILTypeDefForGlobalFunctions ilg (methods, fields) =
    mkILSimpleClass ilg 
        (typeNameForGlobalFunctions, ILTypeDefAccess.Public, methods, fields, 
         emptyILTypeDefs, emptyILProperties, emptyILEvents, emptyILCustomAttrs, 
...
let mkILSimpleModule assname modname dll subsystemVersion useHighEntropyVA tdefs 
    hashalg locale flags exportedTypes metadataVersion =
    { Manifest =
          Some { Name = assname
...
// Detecting Delegates
// -------------------------------------------------------------------- 
let mkILDelegateMethods (ilg : ILGlobals) 
    (iltyp_AsyncCallback, iltyp_IAsyncResult) (parms, rtv : ILReturn) =
    let rty = rtv.Type
    
    let one nm args ret =
        let mdef =
            mkILNonGenericVirtualMethod 
                (nm, ILMemberAccess.Public, args, mkILReturn ret, 
                 MethodBody.Abstract)
        { mdef with mdKind =
                        match mdef.mdKind with
                        | MethodKind.Virtual vinfo -> 
                            MethodKind.Virtual { vinfo with IsAbstract = false }
                        | k -> k
        ...
        mkILCtor 
            (ILMemberAccess.Public, 
             [ mkILParamNamed ("object", ilg.typ_Object)
               mkILParamNamed ("method", ilg.typ_IntPtr) ], MethodBody.Abstract)
    
    let ctor =
        { ctor with mdCodeKind = MethodCodeKind.Runtime
                    IsHideBySig = true }
    
    [ ctor
      one "Invoke" parms rty
      
      one "BeginInvoke" (parms @ [ mkILParamNamed 
                                       ("callback", iltyp_AsyncCallback)
    ...
    mkILInstanceMethSpecInTy 
        (typ, ".ctor", 
         [ rescopeILType scoref ilg.typ_Object
           rescopeILType scoref (if useUIntPtr then ilg.typ_UIntPtr
                                 else ilg.typ_IntPtr) ], ILType.Void, 
         emptyILGenericArgsList)
          ...
          enumValues =
              ...
              |> List.map 
                     ...
                     (fd.Name, 
                      ...
                      | None -> 
                          failwith 
                              ("info_of_enum_tdef: badly formed enum " + mdName 
                               + ": static field does not have an default value"))) }
    | _, [] -> 
        failwith 
            ("info_of_enum_tdef: badly formed enum " + mdName 
             + ": no non-static field found")
    | _, _ -> 
        failwith 
            ("info_of_enum_tdef: badly formed enum " + mdName 
...
let sigptr_get_i64 bytes sigptr =
    ...
    ||| (int64 b4 <<< 32) ||| (int64 b5 <<< 40) ||| (int64 b6 <<< 48) 
    ||| (int64 b7 <<< 56), sigptr
...
let mkRefToILAssembly (m : ILAssemblyManifest) =
    ILAssemblyRef.Create(m.Name, None, 
                         (match m.PublicKey with
                          | Some k -> Some(PublicKey.KeyAsToken(k))
                          | None -> None), m.Retargetable, m.Version, m.Locale)
...
let formatILVersion ((a, b, c, d) : ILVersionInfo) =
    sprintf "%d.%d.%d.%d" (int a) (int b) (int c) (int d)

let encodeCustomAttrString s =
    let arr = string_as_utf8_bytes s
    Array.concat [ z_unsigned_int arr.Length
                   arr ]
...
let rec encodeCustomAttrElemType x =
    ...
    | ILType.Value tspec -> 
        Array.append [| 0x55uy |] 
            (encodeCustomAttrString 
                 tspec.TypeRef.QualifiedNameWithNoShortPrimaryAssembly)
...
let rec encodeCustomAttrElemTypeForObject x =
    ...
    | ILAttribElem.Array(elemTy, _) -> 
        [| yield et_SZARRAY
           yield! encodeCustomAttrElemType elemTy |]
...
let rec decodeCustomAttrElemType (ilg : ILGlobals) bytes sigptr x =
    ...
    | _ -> 
        failwithf 
            "decodeCustomAttrElemType ilg: unrecognized custom element type: %A" 
            x
...
let rec encodeCustomAttrPrimValue ilg c =
    ...
    | ILAttribElem.String None | ILAttribElem.Type None | ILAttribElem.TypeRef None | ILAttribElem.Null -> 
        [| 0xFFuy |]
    ...
    | ILAttribElem.Type(Some ty) -> 
        encodeCustomAttrString ty.QualifiedNameWithNoShortPrimaryAssembly
    | ILAttribElem.TypeRef(Some tref) -> 
        encodeCustomAttrString tref.QualifiedNameWithNoShortPrimaryAssembly
    | ILAttribElem.Array(_, elems) -> 
        [| yield! i32AsBytes elems.Length
           for elem in elems do
               yield! encodeCustomAttrPrimValue ilg elem |]
...
and encodeCustomAttrValue ilg ty c =
    ...
    | ILType.Boxed tspec, _ when tspec.Name = tname_Object -> 
        [| yield! encodeCustomAttrElemTypeForObject c
           yield! encodeCustomAttrPrimValue ilg c |]
    ...
    | ILType.Array(shape, elemType), ILAttribElem.Array(_, elems) when shape = ILArrayShape.SingleDimensional -> 
        [| yield! i32AsBytes elems.Length
           for elem in elems do
               yield! encodeCustomAttrValue ilg elemType elem |]
    | _ -> encodeCustomAttrPrimValue ilg c

let encodeCustomAttrNamedArg ilg (nm, ty, prop, elem) =
    [| yield (if prop then 0x54uy
              else 0x53uy)
       yield! encodeCustomAttrElemType ty
       yield! encodeCustomAttrString nm
       yield! encodeCustomAttrValue ilg ty elem |]

let mkILCustomAttribMethRef (ilg : ILGlobals) 
    (mspec : ILMethodSpec, fixedArgs : list<_>, namedArgs : list<_>) =
...
let mkILCustomAttribute ilg (tref, argtys, argvs, propvs) =
    mkILCustomAttribMethRef ilg 
        (mkILNonGenericCtorMethSpec (tref, argtys), argvs, propvs)
let MscorlibScopeRef =
    ILScopeRef.Assembly
        (ILAssemblyRef.Create
             ("mscorlib", None, Some ecmaPublicKey, true, None, None))
...
let mkPermissionSet (ilg : ILGlobals) 
    (action, 
     attributes : list<ILTypeRef * (string * ILType * ILAttribElem) list>) =
    let bytes =
        [| yield (byte '.')
           yield! z_unsigned_int attributes.Length
           for (tref : ILTypeRef, props) in attributes do
               yield! encodeCustomAttrString 
                          tref.QualifiedNameWithNoShortPrimaryAssembly
               let bytes =
                   [| yield! z_unsigned_int props.Length
                      
                      for (nm, typ, value) in props do
                          yield! encodeCustomAttrNamedArg ilg 
                                     (nm, typ, true, value) |]
...
type ILTypeSigParser(tstring : string) =
    ...
    let peek() =
        if currentPos < (tstring.Length - 1) then tstring.[currentPos + 1]
        else nil
    
    let peekN (skip) =
        if currentPos < (tstring.Length - skip) then tstring.[currentPos + skip]
        else nil
    
    // take a look at the current value, but don't advance
    let here() =
        if currentPos < tstring.Length then tstring.[currentPos]
        else nil
    ...
    let take() =
        let s =
            if currentPos < tstring.Length then tstring.[startPos..currentPos]
            else ""
        drop()
        s
    ...
    member private x.ParseType() =
               ...
               && (peek() <> '[' && peekN (2) <> '[') then 
                ...
                ILScopeRef.Assembly
                    (ILAssemblyRef.FromAssemblyName
                         (System.Reflection.AssemblyName(scope)))
            else ILScopeRef.Local
        ...
        let ilty =
            ...
            | "System.SByte" | "System.Byte" | "System.Int16" | "System.UInt16" | "System.Int32" | "System.UInt32" | "System.Int64" | "System.UInt64" | "System.Char" | "System.Double" | "System.Single" | "System.Boolean" -> 
                ILType.Value(tspec)
...
let decodeILAttribData (ilg : ILGlobals) (ca : ILAttribute) =
    ...
    let rec parseVal argty sigptr =
        ...
        | ILType.Boxed tspec when tspec.Name = "System.Type" -> 
            ...
            | Some n -> 
                ...
                with e -> 
                    failwith 
                        (sprintf 
                             "decodeILAttribData: error parsing type in custom attribute blob: %s" 
                             e.Message)
        | ILType.Boxed tspec when tspec.Name = "System.Object" -> 
            let et, sigptr = sigptr_get_u8 bytes sigptr
            if et = 0xFFuy then ILAttribElem.Null, sigptr
            else 
        ...
        | ILType.Array(shape, elemTy) when shape = ILArrayShape.SingleDimensional -> 
            let n, sigptr = sigptr_get_i32 bytes sigptr
            if n = 0xFFFFFFFF then ILAttribElem.Null, sigptr
            else 
        ...
        | ILType.Value _ -> 
            (* assume it is an enumeration *)
            let n, sigptr = sigptr_get_i32 bytes sigptr
            ILAttribElem.Int32 n, sigptr
        | _ -> 
            failwith 
                "decodeILAttribData: attribute data involves an enum or System.Type value"
        ...
        if n = 0 then List.rev acc
            ...
            let ty, sigptr =
                if (0x50 = (int et) || 0x55 = (int et)) then 
                    let qualified_tname, sigptr =
                        sigptr_get_serstring bytes sigptr
                ...
                else decodeCustomAttrElemType ilg bytes sigptr et
...
let rec refs_of_typ s x =
    ...
    | ILType.Modified(_, ty1, ty2) -> 
        refs_of_tref s ty1
        refs_of_typ s ty2
    | ILType.Array(_, ty) | ILType.Ptr ty | ILType.Byref ty -> refs_of_typ s ty
...
and refs_of_tspec s (x : ILTypeSpec) =
    refs_of_tref s x.TypeRef
    refs_of_inst s x.GenericArgs

and refs_of_callsig s csig =
...
and refs_of_fref s x =
    refs_of_tref s x.EnclosingTypeRef
...
and refs_of_custom_attrs s (cas : ILAttributes) =
    List.iter (refs_of_custom_attr s) cas.AsList
...
and refs_of_instr s x =
    ...
    | I_calli(_, callsig, varargs) -> 
        refs_of_callsig s callsig
        refs_of_varargs s varargs
    | I_jmp mr | I_ldftn mr | I_ldvirtftn mr -> refs_of_mspec s mr
    | I_ldsfld(_, fr) | I_ldfld(_, _, fr) | I_ldsflda fr | I_ldflda fr | I_stsfld(_, 
                                                                                  fr) | I_stfld(_, 
                                                                                                _, 
                                                                                                fr) -> 
        refs_of_fspec s fr
    | I_isinst ty | I_castclass ty | I_cpobj ty | I_initobj ty | I_ldobj(_, _, 
                                                                         ty) | I_stobj(_, 
                                                                                       _, 
                                                                                       ty) | I_box ty | I_unbox ty | I_unbox_any ty | I_sizeof ty | I_ldelem_any(_, 
                                                                                                                                                                 ty) | I_ldelema(_, 
                                                                                                                                                                                 _, 
                                                                                                                                                                                 _, 
                                                                                                                                                                                 ty) | I_stelem_any(_, 
                                                                                                                                                                                                    ty) | I_newarr(_, 
                                                                                                                                                                                                                   ty) | I_mkrefany ty | I_refanyval ty | EI_ilzero ty -> 
        refs_of_typ s ty
    | I_ldtoken token -> refs_of_token s token
    | I_stelem _ | I_ldelem _ | I_ldstr _ | I_switch _ | I_stloc _ | I_stind _ | I_starg _ | I_ldloca _ | I_ldloc _ | I_ldind _ | I_ldarga _ | I_ldarg _ | I_leave _ | I_br _ | I_brcmp _ | I_rethrow | I_refanytype | I_ldlen | I_throw | I_initblk _ | I_cpblk _ | I_localloc | I_ret | I_endfilter | I_endfinally | I_arglist | I_break | AI_add | AI_add_ovf | AI_add_ovf_un | AI_and | AI_div | AI_div_un | AI_ceq | AI_cgt | AI_cgt_un | AI_clt | AI_clt_un | AI_conv _ | AI_conv_ovf _ | AI_conv_ovf_un _ | AI_mul | AI_mul_ovf | AI_mul_ovf_un | AI_rem | AI_rem_un | AI_shl | AI_shr | AI_shr_un | AI_sub | AI_sub_ovf | AI_sub_ovf_un | AI_xor | AI_or | AI_neg | AI_not | AI_ldnull | AI_dup | AI_pop | AI_ckfinite | AI_nop | AI_ldc _ | I_seqpoint _ | EI_ldlen_multi _ -> 
        ()

and refs_of_il_code s (c : ILCode) =
    c.Instrs |> Array.iter (refs_of_instr s)
    c.Exceptions |> List.iter (fun e -> 
                        e.Clause |> (function 
                        | ILExceptionClause.TypeCatch(ilty, _) -> 
                            refs_of_typ s ilty
                        | _ -> ()))
...
and refs_of_events s (x : ILEventDefs) =
    List.iter (refs_of_event_def s) x.AsList
...
and refs_of_properties s (x : ILPropertyDefs) =
    List.iter (refs_of_property_def s) x.AsList
...
and refs_of_exported_types s (tab : ILExportedTypesAndForwarders) =
    List.iter (refs_of_exported_type s) tab.AsList
...
and refs_of_resources s (tab : ILResources) =
    List.iter (refs_of_resource s) tab.AsList
...
let tspan =
    System.TimeSpan
        (System.DateTime.Now.Ticks - System.DateTime(2000, 1, 1).Ticks)
...
let parseILVersion (vstr : string) =
    ...
    if versionComponents.Length > 2 then 
        ...
        let defaultRevision =
            (uint16) (System.DateTime.Now.TimeOfDay.TotalSeconds / 2.0) % System.UInt16.MaxValue - 1us
        if versionComponents.[2] = "*" then 
            if versionComponents.Length > 3 then 
                ...
                vstr <- System.String.Join(".", versionComponents) + "." 
                        + defaultRevision.ToString()
        // Set the revision number to number of seconds today / 2
        else 
            if // since the minor revision will be -1 if none is specified, we need to truncate to 0 to not break existing code
               versionComponents.Length // REVIEW: this uses equality on ILType.  For CMOD_OPTIONAL this is not going to be correct
    ...
    let minorRevision =
        if version.Revision = -1 then 0us
        else uint16 (version.MinorRevision)
    
    (zero32 version.Major, zero32 version.Minor, zero32 version.Build, 
     minorRevision)
...
and unscopeILType typ =
    ...
    | ILType.Modified(b, tref, ty) -> 
        ILType.Modified(b, unscopeILTypeRef tref, unscopeILType ty)
    | x -> x
...
and unscopeILCallSig csig =
    mkILCallSig 
        (csig.CallingConv, unscopeILTypes csig.ArgTypes, 
         unscopeILType csig.ReturnType)
...
let resolveILMethodRefWithRescope r td (mref : ILMethodRef) =
    ...
    | [] -> 
        failwith 
            ("no method named " + nm 
             + " with appropriate argument types found in type " + td.Name)
    | [ mdef ] -> mdef
    | _ -> 
        failwith 
            ("multiple methods named " + nm 
             + " appear with identical argument types in type " + td.Name)
...
let ungenericizeTypeName n =
    let sym = '`'
    if String.contains n sym && (let m = String.rindex n sym
