...
let wordsOfNoArgInstr, isNoArgInstr =
    let t =
        lazy (let t = HashMultiMap(300, HashIdentity.Structural)
              noArgInstrs
              |> Lazy.force
              |> List.iter (fun (x, mk) -> t.Add(mk, x))
              t)
    (fun s -> (Lazy.force t).[s]), (fun s -> (Lazy.force t).ContainsKey s)
#endif
...
(* nb. diff. interp of types to TypeInstr *)
...
let NoArgInstrs =
    lazy (((noArgInstrs
           ...
           @ [ (mk_stind ([ "stind"; "u" ], DT_I))
               ...
               ([ "initblk" ], (fun () -> I_initblk(Aligned, Nonvolatile))) ]) : NoArgInstr InstrTable)
...
let TypeInstrs =
    lazy ([ ([ "ldelema" ], 
             (fun x -> 
             I_ldelema(NormalAddress, false, ILArrayShape.SingleDimensional, x)))
            
            ([ "ldelem"; "any" ], 
             (fun x -> I_ldelem_any(ILArrayShape.SingleDimensional, x)))
            
            ([ "stelem"; "any" ], 
             (fun x -> I_stelem_any(ILArrayShape.SingleDimensional, x)))
            
            ([ "newarr" ], 
             (fun x -> I_newarr(ILArrayShape.SingleDimensional, x)))
...
let IntTypeInstrs =
    lazy ([ ([ "ldelem"; "multi" ], 
             (fun (x, y) -> (I_ldelem_any(ILArrayShape.FromRank x, y))))
            
            ([ "stelem"; "multi" ], 
             (fun (x, y) -> (I_stelem_any(ILArrayShape.FromRank x, y))))
            
            ([ "newarr"; "multi" ], 
             (fun (x, y) -> (I_newarr(ILArrayShape.FromRank x, y))))
            
            ([ "ldelema"; "multi" ], 
             (fun (x, y) -> 
             (I_ldelema(NormalAddress, false, ILArrayShape.FromRank x, y)))) ] : InstrTable<IntTypeInstr>)
