...
let ppenv_enter_method mgparams env = { env with ppenvMethodFormals = mgparams }

let ppenv_enter_tdef gparams env =
    { env with ppenvClassFormals = List.length gparams
...
let rec goutput_scoref _env os =
    function 
    | ILScopeRef.Local -> ()
...
and goutput_typ env os ty =
    ...
    | ILType.Ptr typ -> 
        goutput_typ env os typ
        output_string os "*"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_SByte.TypeSpec.Name -> 
        output_string os "int8"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_Int16.TypeSpec.Name -> 
        output_string os "int16"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_Int32.TypeSpec.Name -> 
        output_string os "int32"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_Int64.TypeSpec.Name -> 
        output_string os "int64"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_IntPtr.TypeSpec.Name -> 
        output_string os "native int"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_Byte.TypeSpec.Name -> 
        output_string os "unsigned int8"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_UInt16.TypeSpec.Name -> 
        output_string os "unsigned int16"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_UInt32.TypeSpec.Name -> 
        output_string os "unsigned int32"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_UInt64.TypeSpec.Name -> 
        output_string os "unsigned int64"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_UIntPtr.TypeSpec.Name -> 
        output_string os "native unsigned int"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_Double.TypeSpec.Name -> 
        output_string os "float64"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_Single.TypeSpec.Name -> 
        output_string os "float32"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_Bool.TypeSpec.Name -> 
        output_string os "bool"
    | ILType.Value tspec when tspec.Name = EcmaMscorlibILGlobals.typ_Char.TypeSpec.Name -> 
...
and goutput_permission _env os p =
    let output_security_action os x =
        output_string os (match x with
                          ...
                          | ILSecurityAction.NonCasLinkDemand -> 
                              "noncaslinkdemand"
                          | ILSecurityAction.NonCasInheritance -> 
                              "noncasinheritance"
                          | ILSecurityAction.LinkDemandChoice -> 
                              "linkdemandchoice"
                          | ILSecurityAction.InheritanceDemandChoice -> 
...
and goutput_security_decls env os (ps : ILPermissions) =
...
and output_bcc os bcc =
    output_string os (match bcc with
...
and output_callconv os (Callconv(hasthis, cc)) =
    output_string os (match hasthis with
...
and goutput_mspec env os (mspec : ILMethodSpec) =
    let fenv =
        ppenv_enter_method mspec.GenericArity 
            (ppenv_enter_tdef (mkILFormalTypars mspec.EnclosingType.GenericArgs) 
                 env)
...
and goutput_vararg_mspec env os (mspec, varargs) =
    ...
    | Some varargs' -> 
        let fenv =
            ppenv_enter_method mspec.GenericArity 
                (ppenv_enter_tdef 
                     (mkILFormalTypars mspec.EnclosingType.GenericArgs) env)
        output_callconv os mspec.CallingConv
        output_string os " "
        goutput_typ fenv os mspec.FormalReturnType
        output_string os " "
...
and goutput_vararg_sig env os (csig : ILCallingSignature, varargs : ILVarArgs) =
    ...
    | Some varargs' -> 
        ...
        if argtys.Length <> 0 then output_seq ", " (goutput_typ env) os argtys
        output_string os ",...,"
...
and goutput_fspec env os (x : ILFieldSpec) =
    let fenv =
        ppenv_enter_tdef (mkILFormalTypars x.EnclosingType.GenericArgs) env
...
let output_member_access os access =
    output_string os (match access with
...
let output_field_init os =
    ...
    | ILFieldInit.Bool x -> 
        output_string os "= bool"
        output_parens output_string os (if x then "true"
                                        else "false")
    | ILFieldInit.Char x -> 
        output_string os "= char"
...
let output_at os b =
    Printf.fprintf os " at (* no labels for data available, data = %a *)" 
        (output_parens output_bytes) b
...
let rec goutput_instr env os inst =
    ...
    | I_brcmp(cmp, tg1) -> 
        output_string os (match cmp with
                          | BI_beq -> "beq"
                          | BI_bgt -> "bgt"
                          | BI_bgt_un -> "bgt.un"
                          | BI_bge -> "bge"
    ...
    | I_newarr(shape, typ) -> 
        if shape = ILArrayShape.SingleDimensional then 
            ...
            let rank = shape.Rank
            output_parens (output_seq "," (goutput_typ env)) os 
                (Array.toList 
                     (Array.create (rank) EcmaMscorlibILGlobals.typ_Int32))
...
let goutput_mbody is_entrypoint env os md =
    ...
    output_string os (if md.IsInternalCall then "internalcall "
                      else " ")
    output_string os (if md.IsManaged then "managed "
                      else " ")
    output_string os (if md.IsForwardRef then "forwardref "
                      else " ")
    output_string os " \n{ \n"
...
let goutput_mdef env os md =
    let attrs =
        ...
        | MethodKind.Virtual vinfo -> 
            "virtual " ^ (if vinfo.IsFinal then "final "
                          else "")
                         ^ (if vinfo.IsNewSlot then "newslot "
                            else "")
                           ^ (if vinfo.IsCheckAccessOnOverride then " strict "
                              else "")
                             ^ (if vinfo.IsAbstract then " abstract "
                                else "")
                               ^ "  "
        | MethodKind.NonVirtual -> ""
        | MethodKind.Ctor -> "rtspecialname"
        | MethodKind.Static -> 
            "static " ^ (match md.mdBody.Contents with
                         | MethodBody.PInvoke(attr) -> 
                             "pinvokeimpl(\"" 
                             ^ attr.Where.Name 
                               ^ "\" as \"" ^ attr.Name ^ "\"" ^ (match attr.CallingConv with
...
let splitTypeLayout =
    ...
    | ILTypeDefLayout.Sequential info -> 
        "sequential", (fun os () -> output_type_layout_info os info)
    | ILTypeDefLayout.Explicit info -> 
        "explicit", (fun os () -> output_type_layout_info os info)
...
let rec goutput_tdef (enc) env contents os cd =
    ...
    if isTypeNameForGlobalFunctions cd.Name then 
        ...
        | ILTypeDefKind.Class | ILTypeDefKind.Enum | ILTypeDefKind.Delegate | ILTypeDefKind.ValueType -> 
            output_string os ".class "
...
let output_modref os (modref : ILModuleRef) =
    output_string os (if modref.HasMetadata then " .module extern "
                      else " .file nometadata ")
...
let goutput_resource env os r =
    ...
    output_string os (match r.Access with
...
let goutput_manifest env os m =
    ...
    | ILAssemblyLongevity.PlatformAppDomain -> 
        output_string os "platformappdomain "
...
let output_module_fragment_aux _refs os modul =
    ...
    with e -> 
        output_string os "*** Error during printing : "
        output_string os (e.ToString())
        os.Flush()
        reraise()
...
let goutput_module_manifest env os modul =
    ...
    output_i32 os ((if modul.IsILOnly then 0x0001
                    else 0) ||| (if modul.Is32Bit then 0x0002
                                 else 0)
                   ||| (if modul.Is32BitPreferred then 0x00020003
                        else 0))
