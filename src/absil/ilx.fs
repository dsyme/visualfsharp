...
type IlxUnionSpec =
    ...
    member x.EnclosingType =
        let (IlxUnionSpec(IlxUnionRef(bx, tref, _, _, _), inst)) = x
        mkILNamedTy bx tref inst
    
    member x.Boxity =
        let (IlxUnionSpec(IlxUnionRef(bx, _, _, _, _), _)) = x
        bx
    
    member x.TypeRef =
        let (IlxUnionSpec(IlxUnionRef(_, tref, _, _, _), _)) = x
        tref
    
    member x.GenericArgs =
        let (IlxUnionSpec(_, inst)) = x
        inst
    
    member x.AlternativesArray =
        let (IlxUnionSpec(IlxUnionRef(_, _, alts, _, _), _)) = x
        alts
    
    member x.IsNullPermitted =
        let (IlxUnionSpec(IlxUnionRef(_, _, _, np, _), _)) = x
        np
    
    member x.HasHelpers =
        let (IlxUnionSpec(IlxUnionRef(_, _, _, _, b), _)) = x
        b
...
let rec instAppsAux n inst =
    function 
    | Apps_tyapp(ty, rty) -> 
        Apps_tyapp(instILTypeAux n inst ty, instAppsAux n inst rty)
    | Apps_app(dty, rty) -> 
        Apps_app(instILTypeAux n inst dty, instAppsAux n inst rty)
    | Apps_done rty -> Apps_done(instILTypeAux n inst rty)

let rec instLambdasAux n inst =
    function 
    | Lambdas_forall(b, rty) -> Lambdas_forall(b, instLambdasAux n inst rty)
    | Lambdas_lambda(p, rty) -> 
        Lambdas_lambda
            ({ p with Type = instILTypeAux n inst p.Type }, 
             instLambdasAux n inst rty)
...
type IlxClosureSpec =
    ...
    member x.TypeRef =
        let (IlxClosureRef(tref, _, _)) = x.ClosureRef
        tref
    
    member x.ILType =
        let (IlxClosureSpec(_, _, ty)) = x
        ty
    
    member x.ClosureRef =
        let (IlxClosureSpec(cloref, _, _)) = x
        cloref
    
    member x.FormalFreeVars =
        let (IlxClosureRef(_, _, fvs)) = x.ClosureRef
        fvs
    
    member x.FormalLambdas =
        let (IlxClosureRef(_, lambdas, _)) = x.ClosureRef
        lambdas
    
    member x.GenericArgs =
        let (IlxClosureSpec(_, inst, _)) = x
        inst
    ...
    member clospec.Constructor =
        ...
        mkILCtorMethSpecForTy (cloTy, 
                               fields
                               |> Array.map (fun fv -> fv.fvType)
                               |> Array.toList)
...
let mkILFormalCloRef gparams csig =
    IlxClosureSpec.Create(csig, mkILFormalGenericArgs 0 gparams)
