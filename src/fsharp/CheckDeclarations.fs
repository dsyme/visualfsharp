...
module IncrClassChecking = 
    ...
    let TcImplicitCtorLhs_Phase2A(cenv: cenv, env, tpenv, tcref: TyconRef, vis, attrs, spats, thisIdOpt, baseValOpt: Val option, safeInitInfo, m, copyOfTyconTypars, objTy, thisTy, doc: PreXmlDoc) =
        ...
        let ctorValScheme, ctorVal = 
            ...
            let ctorValScheme = ValScheme(id, prelimTyschemeG, Some topValInfo, Some memberInfo, false, ValInline.Never, false, NormalVal, vis, false, true, false, false)
        ...
        let cctorValInfo = 
            lazy 
               (let cctorArgs = [ fst(mkCompGenLocal m "unitVar" cenv.g.unit_ty) ]
                ...
                let topValInfo = InferGenericArityFromTyScheme prelimTyschemeG partialValReprInfo
                let cctorValScheme = ValScheme(id, prelimTyschemeG, Some topValInfo, Some memberInfo, false, ValInline.Never, false, NormalVal, Some SynAccess.Private, false, true, false, false)
        ...
        let thisVal = 
            ...
            let thisValScheme = ValScheme(thisId, NonGenericTypeScheme thisTy, None, None, false, ValInline.Never, false, CtorThisVal, None, true, false, false, false)
        ...
        member localRep.ChooseRepresentation (cenv: cenv, env: TcEnv, isStatic, isCtorArg, 
            ...
            let repr = 
                ...
                | topValInfo -> 
                    ...
                    let memberValScheme = ValScheme(id, prelimTyschemeG, Some topValInfo, Some memberInfo, false, ValInline.Never, false, NormalVal, None, true (* isCompilerGenerated *), true (* isIncrClass *), false, false)
                    ...
                    (initialInnerState, binds) ||> List.collectFold (fun innerState defn ->
                        ...
                        | SynMemberDefn.LetBindings (letBinds, isStatic, isRec, m), _ ->
                            ...
                            if not isStatic && tcref.IsStructOrEnumTycon then 
                                let allDo = letBinds |> List.forall (function (SynBinding(_, SynBindingKind.Do, _, _, _, _, _, _, _, _)) -> true | _ -> false)
        ...
        let paramNames =
            ...
            | SynTypeDefnSimpleRepr.General (SynTypeDefnKind.Unspecified, _, _, _, _, _, Some synPats, _) ->
                let rec patName (p: SynSimplePat) =
                    match p with
                    | SynSimplePat.Id (id, _, _, _, _, _, _, _) -> id.idText
            ...
            let members = 
                ...
                let rec preAutoProps memb =
                    ...
                    | SynMemberDefn.AutoProperty(Attributes attribs, isStatic, id, tyOpt, propKind, _, xmlDoc, _access, synExpr, _mGetSet, mWholeAutoProp) -> 
                        ...
                        let isMutable = 
                            ...
                            | _ -> false
                        let headPat = SynPat.LongIdent (LongIdentWithDots([fldId], []), None, Some noInferredTypars, false, isMutable, SynArgPats.Pats [], None, mLetPortion)
                        ...
                        let binding = mkSynBinding (xmlDoc, headPat) (None, mLetPortion, DebugPointAtBinding.NoneAtInvisible, retInfo, synExpr, synExpr.Range, [], attribs, None)
                ...
                let rec postAutoProps memb =
                    ...
                    | SynMemberDefn.AutoProperty(Attributes attribs, isStatic, id, tyOpt, propKind, memberFlags, xmlDoc, access, _synExpr, mGetSetOpt, _mWholeAutoProp) ->
                        ...
                        let headPat = SynPat.LongIdent (LongIdentWithDots(headPatIds, []), None, Some noInferredTypars, false, false, SynArgPats.Pats [], None, mMemberPortion)
                        ...
                        [ 
                            ...
                            | SynMemberKind.PropertyGetSet -> 
                                let getter = 
                                    ...
                                    let binding = mkSynBinding (xmlDoc, headPat) (access, mMemberPortion, DebugPointAtBinding.NoneAtInvisible, retInfo, rhsExpr, rhsExpr.Range, [], attribs, Some (memberFlags SynMemberKind.Member))
                            ...
                            | SynMemberKind.PropertyGetSet -> 
                                let setter = 
                                    ...
                                    let headPat = SynPat.LongIdent (LongIdentWithDots(headPatIds, []), None, Some noInferredTypars, false, false, SynArgPats.Pats [mkSynPatVar None vId], None, mMemberPortion)
                                    let rhsExpr = mkSynAssign (SynExpr.Ident fldId) (SynExpr.Ident vId)
                                    //let retInfo = match tyOpt with None -> None | Some ty -> Some (SynReturnInfo((ty, SynInfo.unnamedRetVal), ty.Range))
                                    let binding = mkSynBinding (xmlDoc, headPat) (access, mMemberPortion, DebugPointAtBinding.NoneAtInvisible, None, rhsExpr, rhsExpr.Range, [], [], Some (memberFlags SynMemberKind.PropertySet))
                                    SynMemberDefn.Member (binding, mMemberPortion) 
                                yield setter 
                            | _ -> ()]
...
let ElimModuleDoBinding bind =
    ...
    | SynModuleDecl.DoExpr (spExpr, expr, m) -> 
        let bind2 = SynBinding (None, SynBindingKind.StandaloneExpression, [], PreXmlDoc.Empty, SynInfo.emptySynValData, SynPat.Wild m, None, expr, m, spExpr)
...
let CheckLetOrDoInNamespace binds m =
    match binds with 
    | [ SynBinding (None, (SynBindingKind.StandaloneExpression | SynBindingKind.Do), [], _, _, _, None, (SynExpr.Do (SynExpr.Const (SynConst.Unit, _), _) | SynExpr.Const (SynConst.Unit, _)), _, _) ] ->
        ()
    | [] -> 
