...
exception ReportedError of exn option with
    override this.Message =
        ...
        match this :> exn with
        | ReportedError(Some exn) -> 
            msg + " Original message: " + exn.Message + ")"
        | _ -> msg
...
exception StopProcessingExn of exn option with
    ...
    override this.ToString() =
        match this :> exn with
        | StopProcessingExn(Some exn) -> 
            "StopProcessingExn, originally (" + exn.ToString() + ")"
        | _ -> "StopProcessingExn"
...
let rec AttachRange m (exn : exn) =
    if m = range0 then exn
        ...
        | :? System.Reflection.TargetInvocationException -> 
            AttachRange m exn.InnerException
        | UnresolvedReferenceNoRange(a) -> UnresolvedReferenceError(a, m)
        | UnresolvedPathReferenceNoRange(a, p) -> 
            UnresolvedPathReference(a, p, m)
        | Failure(msg) -> InternalError(msg ^ " (Failure)", m)
        | :? System.ArgumentException as exn -> 
            InternalError(exn.Message + " (ArgumentException)", m)
...
type PhasedDiagnostic =
    ...
    static member IsSubcategoryOfCompile(subcategory : string) =
        ...
        | BuildPhaseSubcategory.Compile | BuildPhaseSubcategory.Parameter | BuildPhaseSubcategory.Parse | BuildPhaseSubcategory.TypeCheck -> 
            true
        | null | BuildPhaseSubcategory.DefaultPhase | BuildPhaseSubcategory.CodeGen | BuildPhaseSubcategory.Optimize | BuildPhaseSubcategory.IlxGen | BuildPhaseSubcategory.IlGen | BuildPhaseSubcategory.Output | BuildPhaseSubcategory.Interactive -> 
            false
        | BuildPhaseSubcategory.Internal // Getting here means the compiler has ICE-d. Let's not pile on by showing the unknownSubcategory assert below.
        ...
        | unknownSubcategory -> 
            System.Diagnostics.Debug.Assert
                (false, 
                 sprintf 
                     "Subcategory '%s' could not be correlated with a build phase." 
                     unknownSubcategory)
...
type ErrorLogger(nameForDebugging : string) =
    ...
    abstract DiagnosticSink : phasedError:PhasedDiagnostic * isError:bool
     -> unit
...
type internal CompileThreadStatic =
    ...
    static member BuildPhase 
        with get () =
            ...
            | null -> 
                (* assert false; *) BuildPhase.DefaultPhase
...
module ErrorLoggerExtensions =
    ...
    let PreserveStackTrace(exn) =
        ...
        with e -> 
            ...
            System.Diagnostics.Debug.Assert
                (false, "Could not preserve stack trace for watson exception.")
            ()
    ...
    type ErrorLogger with
        ...
        member x.ErrorR exn =
            match exn with
            | InternalError(s, _) | Failure s as exn -> 
                System.Diagnostics.Debug.Assert
                    (false, 
            ...
            | _ -> ()
            match exn with
            | StopProcessing | ReportedError _ -> 
                PreserveStackTrace(exn)
                raise exn
            | _ -> 
                x.DiagnosticSink
                    (PhasedDiagnostic.Create
                         (exn, CompileThreadStatic.BuildPhase), true)
        ...
        member x.Warning exn =
            ...
            | _ -> 
                x.DiagnosticSink
                    (PhasedDiagnostic.Create
                         (exn, CompileThreadStatic.BuildPhase), false)
        ...
        member x.ErrorRecovery (exn : exn) (m : range) =
            ...
            | :? System.Threading.ThreadAbortException | WrappedError((:? System.Threading.ThreadAbortException), 
                                                                      _) -> ()
            ...
            | _ -> 
                ...
                with ReportedError _ | WrappedError(ReportedError _, _) -> ()
...
(* maybe null *) /// NOTE: The change will be undone when the returned "unwind" object disposes
let PushErrorLoggerPhaseUntilUnwind(errorLoggerTransformer : ErrorLogger -> #ErrorLogger) =
    ...
    (*failwith "error logger used after unwind"*)  // REVIEW: ok to throw?
    let chkErrorLogger =
        { new ErrorLogger("PushErrorLoggerPhaseUntilUnwind") with
...
let SetThreadBuildPhaseNoUnwind(phase : BuildPhase) =
    CompileThreadStatic.BuildPhase <- phase
let SetThreadErrorLoggerNoUnwind(errorLogger) =
...
let simulateError (p : PhasedDiagnostic) =
    CompileThreadStatic.ErrorLogger.SimulateError p

let diagnosticSink (phasedError, isError) =
...
let stopProcessingRecovery exn m =
    CompileThreadStatic.ErrorLogger.StopProcessingRecovery exn m
...
let libraryOnlyError m =
    if reportLibraryOnlyFeatures then errorR (LibraryUseOnly(m))

let libraryOnlyWarning m =
    if reportLibraryOnlyFeatures then warning (LibraryUseOnly(m))
...
let conditionallySuppressErrorReporting cond f =
    if cond then suppressErrorReporting f
    else f()
...
let rec RepeatWhileD ndeep body =
    body ndeep ++ (fun x -> 
    if x then RepeatWhileD (ndeep + 1) body
    else CompleteD)
...
let NewlineifyErrorString(message : string) =
    message.Replace
        (stringThatIsAProxyForANewlineInFlatErrors, Environment.NewLine)
