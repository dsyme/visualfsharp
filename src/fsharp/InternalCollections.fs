...
type internal AgedLookup<'Token, 'Key, 'Value when 'Value : not struct>(keepStrongly : int, areSimilar, ?requiredToKeep, ?onStrongDiscard, ?keepMax : int) =
    ...
    let Promote(data, key, value) =
        (data 
         |> List.filter 
                (fun (similarKey, _) -> not (areSimilar (key, similarKey)))) 
        @ [ (key, value) ]
    ...
    let RemoveImpl(data, key) =
        let discard, keep =
            data 
            |> List.partition 
                   (fun (similarKey, _) -> areSimilar (key, similarKey))
        keep, discard
    ...
    let AssignWithStrength(tok, newdata, discard1) =
        ...
        let newdata, discard2 =
            newdata 
            |> List.partition 
                   (fun (n : int, v) -> 
                   n >= tossThreshold || requiredToKeep (snd v))
        
        let newdata =
            newdata |> List.map (fun (n : int, (k, v)) -> 
                           let handle =
    ...
    member al.Put(tok, key, value) =
        ...
        let data, discard =
            if Exists(data, key) then RemoveImpl(data, key)
            else data, []
...
type internal MruCache<'Token, 'Key, 'Value when 'Value : not struct>(keepStrongly, areSame, ?isStillValid : 'Key * 'Value -> bool, ?areSimilar, ?requiredToKeep, ?onStrongDiscard, ?keepMax) =
    ...
    let cache =
        AgedLookup<'Token, 'Key, 'Value>
            (keepStrongly = keepStrongly, areSimilar = areSimilar, 
             ?onStrongDiscard = onStrongDiscard, ?keepMax = keepMax, 
...
type internal List =
    ...
    static member groupByFirst (l : ('Key * 'Value) list) : ('Key * 'Value list) list =
        let nextIndex = ref 0
        let result =
            System.Collections.Generic.List<'Key * System.Collections.Generic.List<'Value>>
                ()
        ...
        for kv in l do
            ...
            if index >= result.Count then 
                ...
                vs.Add(snd kv)
        result
        |> Seq.map (fun (k, vs) -> k, vs |> List.ofSeq)
        |> List.ofSeq
    ...
    static member referenceDistinct (l : 'T list) : 'T list when 'T : not struct =
        let set =
            System.Collections.Generic.Dictionary<'T, bool>
