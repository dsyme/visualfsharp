...
type internal QueueList<'T>(firstElementsIn : 'T list, lastElementsRevIn : 'T list, numLastElementsIn : int) =
    ...
    let firstElements =
        if push then List.append firstElementsIn (List.rev lastElementsRevIn)
        else firstElementsIn
    
    let lastElementsRev =
        if push then []
        else lastElementsRevIn
    
    let numLastElements =
        if push then 0
        else numLastElementsIn
    
    // Compute the last elements on demand.
    let lastElements() =
        if push then []
        else List.rev lastElementsRev
    ...
    member x.ToList() =
        if push then firstElements
        else List.append firstElements (lastElements())
    ...
    member x.Append(ys : seq<_>) =
        QueueList
            (firstElements, (List.rev (Seq.toList ys) @ lastElementsRev), 
             numLastElements + 1)
