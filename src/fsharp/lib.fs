...
let condition _s =
    try 
        (System.Environment.GetEnvironmentVariable(_s) <> null)
    with _ -> false

let GetEnvInteger e dflt =
...
module Bits =
    ...
    let rec pown32 n =
        if n = 0 then 0
        else (pown32 (n - 1) ||| (1 <<< (n - 1)))
    
    let rec pown64 n =
        if n = 0 then 0L
        else (pown64 (n - 1) ||| (1L <<< (n - 1)))
...
module Filename =
    let fullpath cwd nm =
        let p =
            if FileSystem.IsPathRootedShim(nm) then nm
            else Path.Combine(cwd, nm)
        try 
            FileSystem.GetFullPathShim(p)
        with :? System.ArgumentException | :? System.ArgumentNullException | :? System.NotSupportedException | :? System.IO.PathTooLongException | :? System.Security.SecurityException -> 
            p
    
    let hasSuffixCaseInsensitive suffix filename =
        (* case-insensitive *)
        Filename.checkSuffix (String.lowercase filename) 
            (String.lowercase suffix)
...
module NameMap =
    let domain m =
        Map.foldBack (fun x _ acc -> Zset.add x acc) m (Zset.empty String.order)
...
module Check =
    ...
    let StringArgumentNotNullOrEmpty (s : string) argname =
        ...
        if s.Length = 0 then raise (new System.ArgumentNullException(argname))
...
module ListAssoc =
    ...
    /// function passed in.
    let rec containsKey (f : 'key -> 'key -> bool) (x : 'key) 
            (l : ('key * 'value) list) : bool =
        match l with
        | [] -> false
...
module ListSet =
    ...
    let unionFavourRight f l1 l2 =
        ...
        | _, [] -> l1
        | [], _ -> l2
        | _ -> List.foldBack (insert f) l1 l2
    
    (* nb. foldBack to preserve natural orders *)
    ...
    let isSupersetOf f l1 l2 =
        List.forall (fun x2 -> contains (fun y2 y1 -> f y1 y2) x2 l1) l2
    ...
    let rec intersect f l1 l2 =
        match l2 with
        | (h :: t) -> 
            if contains f h l1 then h :: intersect f l1 t
            else intersect f l1 t
        | [] -> []
...
// Library: pairs
//------------------------------------------------------------------------
let mapFoldFst f s (x, y) =
    let x', s = f s x
    (x', y), s

let mapFoldSnd f s (x, y) =
    let y', s = f s y
    (x, y'), s
...
let foldQuadruple (f1, f2, f3, f4) acc (a1, a2, a3, a4) =
    f4 (f3 (f2 (f1 acc a1) a2) a3) a4
let mapPair (f1, f2) (a1, a2) = (f1 a1, f2 a2)
let mapTriple (f1, f2, f3) (a1, a2, a3) = (f1 a1, f2 a2, f3 a3)
let mapQuadruple (f1, f2, f3, f4) (a1, a2, a3, a4) =
    (f1 a1, f2 a2, f3 a3, f4 a4)

let fmap2Of2 f z (a1, a2) =
    let z, a2 = f z a2
    z, (a1, a2)
...
module Zmap =
    let force k mp =
        match Zmap.tryFind k mp with
        | Some x -> x
        | None -> failwith "Zmap.force: lookup failed"
...
type Graph<'Data, 'Id when 'Id : comparison and 'Id : equality>(nodeIdentity : 'Data -> 'Id, nodes : 'Data list, edges : ('Data * 'Data) list) =
    let edges =
        edges |> List.map (fun (v1, v2) -> nodeIdentity v1, nodeIdentity v2)
    
    let nodes =
        nodes |> List.map (fun d -> 
                     nodeIdentity d, 
                     { nodeId = nodeIdentity d
                       nodeData = d
                       nodeNeighbours = [] })
    ...
    do 
        for node in nodes do
            node.nodeNeighbours <- edges
                                   |> List.filter 
                                          (fun (x, _y) -> x = node.nodeId)
                                   |> List.map (fun (_, nodeId) -> tab.[nodeId])
    
    member g.GetNodeData nodeId = tab.[nodeId].nodeData
    member g.IterateCycles f =
        let rec trace path node =
            if List.exists (nodeIdentity >> (=) node.nodeId) path then 
                f (List.rev path)
...
//---------------------------------------------------------------------------
type cache<'T> =
    { mutable cacheVal : 'T NonNullSlot }
    ...
    type AsyncResultCell<'T>() =
        ...
        member x.RegisterResult(res : AsyncResult<'T>) =
            ...
            let postOrQueue (sc : SynchronizationContext, cont) =
                match sc with
                | null -> 
                    ThreadPool.QueueUserWorkItem(fun _ -> cont res) |> ignore
            ...
            | [] -> ()
            | [ (sc, cont) as c ] -> 
                if SynchronizationContext.Current = sc then cont res
                else postOrQueue c
            | _ -> grabbedConts |> List.iter postOrQueue
        ...
        member x.AsyncResult = async { let! res = x.AsyncPrimitiveResult
...
module UnmanagedProcessExecutionOptions =
    ...
    [<DllImport("kernel32.dll")>]
    extern bool HeapSetInformation(UIntPtr _HeapHandle, UInt32 _HeapInformationClass, UIntPtr _HeapInformation, UIntPtr _HeapInformationLength)
    ...
    let EnableHeapTerminationOnCorruption() =
        if (System.Environment.OSVersion.Version.Major >= 6 
            ...
            let HeapEnableTerminationOnCorruption : uint32 = 1u
            if not 
                   (HeapSetInformation
                        (GetProcessHeap(), HeapEnableTerminationOnCorruption, 
                         UIntPtr.Zero, UIntPtr.Zero)) then 
                raise 
