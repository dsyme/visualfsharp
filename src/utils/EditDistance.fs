...
let jaro s1 s2 =
    ...
    let commonChars (chars1 : string) (chars2 : string) =
        let rec inner i result =
            ...
            | -1 -> result
            | _ -> 
                if existsInWin chars1.[i] chars2 i matchRadius then 
                    inner (i - 1) (chars1.[i] :: result)
    ...
    let result =
        (c1length / s1length + c2length / s2length 
         + (tLength - transpositions) / tLength) / 3.0
    // This is for cases where |s1|, |s2| or m are zero 
    if System.Double.IsNaN result then 0.0
    else result
...
let CalcEditDistance(a : string, b : string) =
    if a.Length > b.Length then calcDamerauLevenshtein (a, b)
    else calcDamerauLevenshtein (b, a)
