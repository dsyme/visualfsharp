...
type internal HashMultiMap<'Key, 'Value>(n : int, hasheq : IEqualityComparer<'Key>) =
    ...
    new(hasheq : IEqualityComparer<'Key>) =
        HashMultiMap<'Key, 'Value>(11, hasheq)
    ...
    member x.Add(y, z) =
        ...
        if ok then rest.[y] <- res :: x.GetRest(y)
        firstEntries.[y] <- z
    ...
    member x.Copy() =
        let res =
            HashMultiMap<'Key, 'Value>
                (firstEntries.Count, firstEntries.Comparer)
        ...
        if ok then 
            ...
            if ok then 
                ...
                | (h :: t) -> 
                    ...
                    rest.[y] <- t
                | _ -> ()
            else firstEntries.Remove(y) |> ignore
    ...
    interface IDictionary<'Key, 'Value> with
        ...
        member s.Keys =
            ([| for kvp in s -> kvp.Key |] :> ICollection<'Key>)
        
        member s.Values =
            ([| for kvp in s -> kvp.Value |] :> ICollection<'Value>)
    ...
    interface ICollection<KeyValuePair<'Key, 'Value>> with
        ...
        member s.Remove(x) =
            ...
            if res && Unchecked.equals s.[x.Key] x.Value then s.Remove(x.Key)
            res
