...
type internal HashMultiMap<'Key,'Value> =
    ...
    /// Remove all values for a particular key from the collection.
    member RemoveAll: 'Key -> unit
    
    /// Set all bindings for a particular key in the collection. The latest binding should
    /// come first in the list.
    member SetAll: 'Key * 'Value list -> unit
    ...
    member RemoveLatest: 'Key -> unit

    /// Replace the latest binding if any for the given element.
    member ReplaceLatest: 'Key * 'Value -> unit
    ...
    /// Find all bindings for the given element in the table, if any. The latest binding
