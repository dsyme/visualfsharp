...
module internal ResizeArray =
    ...
    let append (arr1 : ResizeArray<'T>) (arr2 : ResizeArray<'T>) =
        concat [ arr1; arr2 ]
    ...
    let sub (arr : ResizeArray<'T>) start len =
        ...
        new ResizeArray<_>(seq { 
                               for i in start..start + len - 1 -> arr.[i]
                           })
    ...
    let foldBack (f : 'T -> 'State -> 'State) (arr : ResizeArray<'T>) 
        (acc : 'State) =
        let mutable res = acc
        let len = length arr
        for i = len - 1 downto 0 do
    ...
    let foldBack2 f (arr1 : ResizeArray<'T1>) (arr2 : ResizeArray<'T2>) 
        (acc : 'b) =
        let f = FSharpFunc<_, _, _, _>.Adapt(f)
        let mutable res = acc
        let len = length arr1
    ...
    let mapi2 (f : int -> 'T -> 'b -> 'c) (arr1 : ResizeArray<'T>) 
        (arr2 : ResizeArray<'b>) =
        let f = FSharpFunc<_, _, _, _>.Adapt(f)
        let len1 = length arr1
        if len1 <> length arr2 then 
