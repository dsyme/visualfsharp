...
module SetTree =
    ...
    let rec filterAux comparer f s acc =
        ...
        | SetNode(k, l, r, _) -> 
            let acc =
                if f k then add comparer k acc
                else acc
    ...
    let rec diffAux comparer m acc =
        match m with
        | SetNode(k, l, r, _) -> 
            diffAux comparer l (diffAux comparer r (remove comparer k acc))
        | SetEmpty -> acc
    ...
    let rec union comparer t1 t2 =
        ...
        | SetNode(k1, t11, t12, h1), SetNode(k2, t21, t22, h2) -> // (t11 < k < t12) AND (t21 < k2 < t22) 
            ...
            if h1 > h2 then 
                let lo, _, hi = split comparer k1 t2
                balance comparer (union comparer t11 lo) k1 
                    (union comparer t12 hi)
            else 
                let lo, _, hi = split comparer k2 t1
                balance comparer (union comparer t21 lo) k2 
                    (union comparer t22 hi)
        | SetEmpty, t -> t
        | t, SetEmpty -> t
    ...
    let rec intersectionAux comparer b m acc =
        ...
        | SetNode(k, l, r, _) -> 
            ...
            let acc =
                if contains comparer k b then add comparer k acc
                else acc
    ...
    let rec nextElemCont (comparer : IComparer<'T>) k s cont =
        ...
        | MatchSetNode(k2, l, r) -> 
            let c = comparer.Compare(k, k2)
            if c < 0 then 
                nextElemCont comparer k l (function 
                    | None -> cont (Some(k2))
                    | res -> res)
    ...
    and prevElemCont (comparer : IComparer<'T>) k s cont =
        ...
        | MatchSetNode(k2, l, r) -> 
            let c = comparer.Compare(k, k2)
            if c > 0 then 
                prevElemCont comparer k r (function 
                    | None -> cont (Some(k2))
                    | res -> res)
    ...
    type SetIterator<'T>(s : SetTree<'T>) =
        ...
        let rec collapseLHS stack =
            ...
            | SetNode(k, l, r, _) :: rest -> 
                collapseLHS (l :: SetOne k :: r :: rest)
        ...
        let notStarted() =
            raise 
                (new System.InvalidOperationException("Enumeration has not started. Call MoveNext."))
        let alreadyFinished() =
            raise 
                (new System.InvalidOperationException("Enumeration already finished."))
    ...
    let rec compareStacks (comparer : IComparer<'T>) l1 l2 =
        ...
        | (SetNode(n1k, SetEmpty, n1r, _) :: t1), 
          (SetNode(n2k, SetEmpty, n2r, _) :: t2) -> 
            let c = comparer.Compare(n1k, n2k)
            if c <> 0 then c
            else compareStacks comparer (n1r :: t1) (n2r :: t2)
...
type internal Set<'T, 'ComparerTag when 'ComparerTag :> IComparer<'T>>(comparer : IComparer<'T>, tree : SetTree<'T>) =
    ...
    member s.Add(x) : Set<'T, 'ComparerTag> =
        refresh s (SetTree.add comparer x tree)
    ...
    static member Intersection((a : Set<'T, 'ComparerTag>), 
                               (b : Set<'T, 'ComparerTag>)) : Set<'T, 'ComparerTag> =
        match b.Tree with
    ...
    static member Difference(a : Set<'T, 'ComparerTag>, 
                             b : Set<'T, 'ComparerTag>) : Set<'T, 'ComparerTag> =
        match a.Tree with
        | SetEmpty -> a // 0 - B = 0 
        | _ -> 
    ...
    member s.IsSubsetOf((y : Set<'T, 'ComparerTag>)) =
        SetTree.subset comparer tree y.Tree
    member s.IsSupersetOf((y : Set<'T, 'ComparerTag>)) =
        SetTree.subset comparer y.Tree tree
    ...
    interface System.IComparable with
        ...
        member s1.CompareTo(s2 : obj) =
            SetTree.compare s1.Comparer s1.Tree 
                ((s2 :?> Set<'T, 'ComparerTag>).Tree)
...
module MapTree =
        ...
        if t2h > t1h + 2 then // right is heavier than left 
            ...
            | MapNode(t1k, t1v, t1l, t1r, _) -> 
                ...
                if height t1r > t2h + 1 then 
                    ...
                    | _ -> failwith "rebalance"
                else mk t1l t1k t1v (mk t1r k v t2)
            | _ -> failwith "rebalance"
        else mk t1 k v t2
    ...
    let indexNotFound() =
        raise 
            (new System.Collections.Generic.KeyNotFoundException("An index satisfying the predicate was not found in the collection"))
    ...
    let partition1 (comparer : IComparer<'T>) f k v (acc1, acc2) =
        if f k v then (add comparer k v acc1, acc2)
        else (acc1, add comparer k v acc2)
    ...
    let partition (comparer : IComparer<'T>) f s =
        partitionAux comparer f s (empty, empty)
    
    let filter1 (comparer : IComparer<'T>) f k v acc =
        if f k v then add comparer k v acc
        else acc
    ...
    let rec map f m =
        ...
        | MapNode(k, v, l, r, h) -> 
            let v2 = f v
            MapNode(k, v2, map f l, map f r, h)
    ...
    let rec mapi f m =
        ...
        | MapNode(k, v, l, r, h) -> 
            let v2 = f k v
            MapNode(k, v2, mapi f l, mapi f r, h)
    ...
    let foldSection (comparer : IComparer<'T>) lo hi f m x =
        let rec fold_from_to f m x =
            ...
            | MapNode(k, v, l, r, _) -> 
                ...
                let x =
                    if clo_k < 0 then fold_from_to f l x
                    else x
                
                let x =
                    if clo_k <= 0 && ck_hi <= 0 then f k v x
                    else x
                
                let x =
                    if ck_hi < 0 then fold_from_to f r x
                    else x
                
                x
        if comparer.Compare(lo, hi) = 1 then x
        else fold_from_to f m x
    ...
    type MapIterator<'Key, 'T>(s : MapTree<'Key, 'T>) =
        ...
        let rec collapseLHS stack =
            ...
            | (MapNode(k, v, l, r, _)) :: rest -> 
                collapseLHS (l :: MapOne(k, v) :: r :: rest)
        ...
        member i.Current =
            if started then 
                match stack with
                | (MapNode(k, v, MapEmpty, MapEmpty, _)) :: _ -> 
                    new KeyValuePair<_, _>(k, v)
                | [] -> alreadyFinished()
                | _ -> 
                    failwith 
    ...
    interface System.IComparable with
        member m1.CompareTo(m2 : obj) =
            Seq.compareWith (fun (kvp1 : KeyValuePair<_, _>) (kvp2 : KeyValuePair<_, _>) -> 
                let c = m1.Comparer.Compare(kvp1.Key, kvp2.Key)
                if c <> 0 then c
                else Unchecked.compare kvp1.Value kvp2.Value) // Cast m2 to the exact same type as m1, see 4884.
