...
let checkPathForIllegalChars =
    let chars =
        new System.Collections.Generic.HashSet<_>(Path.GetInvalidPathChars())
    (fun (path : string) -> 
    for c in path do
        if chars.Contains c then raise (IllegalFileNameChar(path, c)))

// Case sensitive (original behaviour preserved).
let checkSuffix (x : string) (y : string) =
    x.EndsWith(y, System.StringComparison.Ordinal)
...
let chopExtension (s : string) =
    ...
    if s = "." then ""
        ...
        Path.Combine
            (Path.GetDirectoryName s, Path.GetFileNameWithoutExtension(s))
...
let directoryName (s : string) =
    ...
    if s = "" then "."
        ...
        | null -> 
            if FileSystem.IsPathRootedShim(s) then s
            else "."
        | res -> 
            if res = "" then "."
            else res
