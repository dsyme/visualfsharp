...
type internal Position =
    ...
    new(fileIndex : int, line : int, originalLine : int, 
        startOfLineAbsoluteOffset : int, absoluteOffset : int) =
        { FileIndex = fileIndex
          Line = line
    ...
    member x.NextLine =
        Position
            (x.FileIndex, x.Line + 1, x.OriginalLine + 1, x.AbsoluteOffset, 
             x.AbsoluteOffset)
    member x.EndOfToken n =
        Position
            (x.FileIndex, x.Line, x.OriginalLine, x.StartOfLineAbsoluteOffset, 
             x.AbsoluteOffset + n)
    ...
    member x.ApplyLineDirective(fileIdx, line) =
        Position
            (fileIdx, line, x.OriginalLine, x.AbsoluteOffset, x.AbsoluteOffset)
    static member Empty = Position()
    static member FirstLine fileIdx = Position(fileIdx, 1, 0, 0, 0)

type internal LexBufferFiller<'Char> = LexBuffer<'Char> -> unit

and [<Sealed>] internal LexBuffer<'Char>(filler : LexBufferFiller<'Char>) =
    ...
    member lexbuf.LexemeLength 
        with get () = lexemeLength : int
        and set v = lexemeLength <- v
    
    member lexbuf.Buffer 
        with get () = buffer : // A full type signature is required on this method because it is used at more specific types within its own scope
                               'Char []
    ...
    static member LexemeString(lexbuf : LexBuffer<char>) =
        new System.String(lexbuf.Buffer, lexbuf.BufferScanStart, 
                          lexbuf.LexemeLength)
    ...
    member lexbuf.EnsureBufferSize n =
        if lexbuf.BufferScanPos + n >= buffer.Length then 
            ...
            Array.blit buffer bufferScanStart repl bufferScanStart 
                bufferScanLength
            buffer <- repl
...
module GenericImplFragments =
    let startInterpret (lexBuffer : LexBuffer<char>) =
        lexBuffer.BufferScanStart <- lexBuffer.BufferScanStart 
                                     + lexBuffer.LexemeLength
        lexBuffer.BufferMaxScanLength <- lexBuffer.BufferMaxScanLength 
                                         - lexBuffer.LexemeLength
        lexBuffer.BufferScanLength <- 0
        lexBuffer.LexemeLength <- 0
        lexBuffer.BufferAcceptAction <- -1
    
    let afterRefill (trans : uint16 [] [], sentinel, lexBuffer : LexBuffer<char>, 
                     scanUntilSentinel, endOfScan, state, eofPos) =
        if lexBuffer.BufferScanLength = lexBuffer.BufferMaxScanLength then 
            let snew = int trans.[state].[eofPos]
            if snew = sentinel then endOfScan()
            else 
        ...
        else scanUntilSentinel lexBuffer state
...
type internal UnicodeTables(trans : uint16 [] array, accept : uint16 []) =
    ...
    let lookupUnicodeCharacters state inp =
        ...
        if inpAsInt < numLowUnicodeChars then int trans.[state].[inpAsInt]
        else 
            let baseForSpecificUnicodeChars = numLowUnicodeChars
            
            let rec loop i =
                if i >= numSpecificUnicodeChars then 
                    let baseForUnicodeCategories =
                        numLowUnicodeChars + numSpecificUnicodeChars * 2
                    let unicodeCategory = System.Char.GetUnicodeCategory(inp)
                    int 
                        trans.[state].[baseForUnicodeCategories 
                                       + int32 unicodeCategory]
                else 
                    let c =
                        char 
                            (int 
                                 trans.[state].[baseForSpecificUnicodeChars 
                                                + i * 2])
                    if c = inp then 
                        int 
                            trans.[state].[baseForSpecificUnicodeChars + i * 2 
                                           + 1]
                    else loop (i + 1)
            loop 0
    
    let eofPos =
        numLowUnicodeChars + 2 * numSpecificUnicodeChars + numUnicodeCategories
    ...
    let rec scanUntilSentinel lexBuffer state =
        ...
        if lexBuffer.BufferScanLength = lexBuffer.BufferMaxScanLength then 
            ...
            afterRefill 
                (trans, sentinel, lexBuffer, scanUntilSentinel, 
                 lexBuffer.EndOfScan, state, eofPos)
        else 
            let inp = lexBuffer.Buffer.[lexBuffer.BufferScanPos]
            let snew = lookupUnicodeCharacters state inp
            if snew = sentinel then lexBuffer.EndOfScan()
            else 
