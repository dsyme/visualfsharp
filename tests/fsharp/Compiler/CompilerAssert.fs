...
let main argv = 0"""
    ...
    let TypeCheckWithErrorsAndOptions options (source: string) expectedTypeErrors =
        TypeCheckWithErrorsAndOptionsAndAdjust options 0 (source: string) expectedTypeErrors
    ...
    let TypeCheckSingleErrorWithOptions options (source: string) (expectedSeverity: FSharpErrorSeverity) (expectedErrorNumber: int) (expectedErrorRange: int * int * int * int) (expectedErrorMsg: string) =
        TypeCheckWithErrorsAndOptions options source [| expectedSeverity, expectedErrorNumber, expectedErrorRange, expectedErrorMsg |]
